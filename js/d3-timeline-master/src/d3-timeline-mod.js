        ...
        g.each(function (d, i) {
          d.forEach(function (datum, index) {
            ...
            if (ending === 0 && beginning === 0){
              datum.encounters.forEach(function (date, i) {
                if (encounter.date.starting_time < minTime || (minTime === 0 && timeIsRelative === false))
                  minTime = date.starting_time;
                if (encounter.date.ending_time > maxTime)
                  maxTime = date.ending_time;
              });
            }
          });
        });
      ...
      if (showBorderLine) {
        g.each(function (d, i) {
          d.forEach(function (datum) {
            var times = datum.encountersTimes;
